...
function taskSubmit(){
	...
	if(dataType=="tj"){
		obj.tjMc = $("#tjMcOld").val();
		obj.tjRemark=$("#tjRemarkOld").val();
		url="/sjcq/photoTjManage/contributorTaskSubmit";
	}else{
		obj.picMc=$("#picMcOld").val();
		obj.picRemark=$("#picRemarkOld").val();
		obj.picFRemark=$("#picFRemarkOld").val();
		obj.picDd=$("#picDdOld").val();
		obj.picSyz=$("#picSyzOld").val();
		if($("#picSysjOld").val()&&$("#picSysjOld").val().length>0){
		//	alert($("#picSysj").val());
			obj.picSysj=new Date($("#picSysjOld").val().trim());
		}
		obj.picGjz=$("#picGjzOld").val();
		obj.picMj=$("#picMjOld").val();
		obj.picType=$("#picTypeOld").val();
		obj.typeOne="";
		obj.typeTwo="";
		obj.typeThree="";
		obj.typeFour="";
		...
		$.each($("#contentUploadPicSortOld").find(".sct"),function(index,item){
			console.log();
		...
		url="/sjcq/photoPicManage/contributorTaskSubmit";
	}
	//提交审核
	$box.promptSureBox("是否确定修改供稿人提交的信息，并提交审核任务！", "sureSubmit", url)
}
